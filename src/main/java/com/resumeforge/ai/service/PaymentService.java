...
public class PaymentService {
    ...
    // PAY-01 FIX: Amount (paise) and currency are injected from application.properties /
    // environment variables (PAYMENT_AMOUNT, PAYMENT_CURRENCY).  Nothing is hardcoded here.
    ...
    public Map<String, Object> createPaymentOrder(User user, PaymentCreateRequest request) {
        try {
            ...
            // Use env-configured paise value; convert to rupees for the Payment entity.
            int    amountPaise  = configuredAmountPaise;
            String currency     = configuredCurrency;
            BigDecimal amount   = BigDecimal.valueOf(amountPaise).divide(new BigDecimal("100"));
